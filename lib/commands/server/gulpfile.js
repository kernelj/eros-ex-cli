...
gulp.task('handle-config-native', (done) => {
    ...
    nativeConfig.ip = IP
    if (!nativeConfig.url.jsServer && !isPack) {
        nativeConfig.url.jsServer = `${isSSL ? 'https' : 'http'}://${IP}:${devPort}`
    }
    // pack 时候做兼容
    if (!nativeConfig.url.jsServer && isPack) {
        nativeConfig.url.jsServer = `http://${IP}:${server['port']}`
    }    
...
gulp.task('dev', (callback) => {
    gulpSequence('clean', ['build-assets', 'build-mock', 'build-js'], ['start-mock', 'handle-config-native', "compatible-weex-debug", 'watch', 'socket'])(callback)
});
